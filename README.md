...
| [04 · Kubernetes Fundamentals](notes/04-kubernetes-fundamentals/) | Orchestrators and the market; kubeadm architecture — runc, containerd, kubelet, static pods, etcd, API server, scheduler, controller managers, kube-proxy, CoreDNS, HA; Pods as the smallest deployable unit — shared network namespace, one Pod IP, sidecars and init containers; kubectl from `run` to `explain`, imperative vs declarative; troubleshooting — phases vs the STATUS column, `describe`, events, logs and exec; namespaces — the four defaults, namespaced vs cluster-scoped, ResourceQuota vs LimitRange, RBAC scoping and kubeconfig contexts; Deployments and ReplicaSets — the ownership chain, pod-template-hash, rolling updates, revisions and rollback; DaemonSets — node coverage versus replica count, and the taints that decide which nodes are eligible; `kubectl set image` and `kubectl patch` — strategic merge vs JSON merge patch vs JSON Patch; Services — the four types, endpoints and EndpointSlices, headless Services and cluster DNS; Jobs and CronJobs — run-to-completion workloads, completions versus parallelism, cron schedules and cascading deletion; ConfigMaps — creation sources, consumption methods and update behaviour; Secrets — encoded not encrypted, the built-in types, TLS Secrets and what actually protects them; labels and selectors — the mechanism wiring Services, controllers, policies and scheduling together; annotations — the non-selectable counterpart and the signalling channel for Ingress controllers, cert-manager and service meshes; startup, liveness and readiness probes — how they gate each other and what each failure does | In progress |
